...
with open('model_accuracies.txt','w') as file:
    for k in range(15):
        ...
        if 0 <= k <= 2:
            dqn.load_state_dict(torch.load(f'./softdqn_model{num}.pth'))
        elif 3 <= k <= 5:
            dqn.load_state_dict(torch.load(f'./ddqn_model{num}.pth'))
        elif 6 <= k <= 8:
            dqn.load_state_dict(torch.load(f'./Nosiydqn_model{num}.pth'))
        elif 9 <= k <= 11:
            dqn.load_state_dict(torch.load(f'./EREdqn_model{num}.pth'))
        elif 12 <= k <= 14:
            dqn.load_state_dict(torch.load(f'./PERdqn_model{num}.pth'))
        ...
        for l in range(10):
            ...
            for j in range(10):
                ...
                for episode in range(num_episodes):
                    ...
                    action = select_action(dqn, state, epsilon=0)
            ...
            if 0 <= k <= 2:
                text2 = f"The model {num:.0f} softdqn {l:.0f} accuracy is {formatted_dqn_a}%"
            elif 3 <= k <= 5:
                text2 = f"The model {num:.0f} ddqn {l:.0f} accuracy is {formatted_dqn_a}%"
            elif 6 <= k <= 8:
                text2 = f"The model {num:.0f} Nosiydqn {l:.0f} accuracy is {formatted_dqn_a}%"
            elif 9 <= k <= 11:
                text2 = f"The model {num:.0f} EREdqn {l:.0f} accuracy is {formatted_dqn_a}%"
            elif 12 <= k <= 14:
                text2 = f"The model {num:.0f} PERdqn {l:.0f} accuracy is {formatted_dqn_a}%"
