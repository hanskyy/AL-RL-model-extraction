...
def fgsm_attack(image, epsilon, data_grad):
    ...
    :param epsilon: The amount of perturbation allowed (controls the strength of the attack).
    :param data_grad: Gradients of the loss with respect to the image (tensor).
...
def retrain_cnn_models(trained_cnns, optimizers, selected_image, selected_label):
    ...
    epsilon = 0.25  # Strength of the attack
    ...
    for model, optimizer in zip(trained_cnns, optimizers):
        ...
        # Generate adversarial examples on the original data
        data_grad = selected_image.grad.data
        perturbed_data = fgsm_attack(selected_image, epsilon, data_grad)
...
with open('model_accuracies.txt','w') as file:
    for k in range(3):
        ...
        text2 = f"The model {num:.0f} FGSM accuracy is {formatted_dqn_a}%"
