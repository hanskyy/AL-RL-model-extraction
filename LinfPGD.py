...
def linf_pgd_attack(model, images, labels, epsilon=0.3, alpha=0.01, iters=40, random_start=True):
    """
    Performs the Linf PGD attack on a batch of images.

    :param model: The model to attack.
    :param images: Original images (batch).
    :param labels: True labels for the images.
    :param eps: Attack perturbation magnitude (epsilon).
    :param alpha: Step size for each iteration.
    :param iters: Number of iterations.
    :param random_start: Whether to start with a random perturbation.
    :return: Perturbed images.
    """
    # Clone the original images to avoid altering them
    perturbed_images = images.clone()
    perturbed_images.requires_grad = True

    # If random_start, initialize the perturbations randomly within the allowed epsilon ball
    if random_start:
    ...
    for _ in range(iters):
        ...
        # Calculate the L_inf norm gradient and scale the gradient to have its components in [-alpha, alpha]
        grad = perturbed_images.grad.data.sign() * alpha

        # Update the images by the calculated gradient
        perturbed_images = perturbed_images + grad

        # Project the perturbed images back into the epsilon-ball and ensure they are valid images
...
def retrain_cnn_models(trained_cnns, optimizers, selected_image, selected_label):
    ...
    for model, optimizer in zip(trained_cnns, optimizers):
        ...
        perturbed_data = linf_pgd_attack(model, selected_image,selected_label, epsilon)
...
with open('model_accuracies.txt','w') as file:
    for k in range(3):
        ...
        text2 = f"The model {num:.0f} LinfPGD accuracy is {formatted_dqn_a}%"
