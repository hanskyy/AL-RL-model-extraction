...
class ReplayBuffer(object):
    ...
    def sample(self, batch_size, beta=0.4, c_k=0):
        if len(self.memory) == self.capacity:
            prios = self.priorities
            if c_k == 0:
                c_k = len(self.memory)
            elif c_k == 1:
                c_k = len(self.memory) / 2
                c_k = int(c_k)
                prios = self.priorities[:c_k]

            else:
                c_k = len(self.memory) / 4
                c_k = int(c_k)
                prios = self.priorities[:c_k]
        else:
...
def update_dqn(model, target_model, optimizer, criterion, replay_buffer, batch_size, beta, c_k, gamma=0.99):
    """
    ...
    minibatch, indices, weights = replay_buffer.sample(batch_size, beta, c_k)
...
for i in range(3):
    ...
    for episode in range(num_episodes):
        ...
        for t in range(1, max_steps_per_episode):
            ...
            if len(replay_buffer) > batch_size:
                c_k = episode
                update_dqn(dqn, target_dqn, optimizer, criterion, replay_buffer, batch_size, beta, c_k)
    ...
    torch.save(dqn.state_dict(),f'./EREdqn_model{num_episodes}.pth')